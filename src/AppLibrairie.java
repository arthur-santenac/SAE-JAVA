...
public void menuLivreRecommande() throws SQLException {
    List<Livre> livresRecommandes = clientBD.onVousRecommande(utilisateur.getIdCli());

    System.out.println("     _______________________________________________________________   _______________________________________________________________  ");
    System.out.println(" .-/|                                                               \\ /                                                               |\\-.");
    System.out.println(" ||||                   LIVRES RECOMMANDÉS                           |                                                                ||||");
    System.out.println(" ||||________________________________________________________________|                                                                ||||");

    for (Livre livre : livresRecommandes) {
        String ligne = String.format(" ||||  %-65s|", 
            String.format("Titre: %s | Prix: %.2f €", livre.getTitre(), livre.getPrix()));
        System.out.println(ligne + "                                                                                                                          ||||");
    }

    int lignesAffichees = livresRecommandes.size();
    for (int i = lignesAffichees; i < 30; i++) {
        System.out.println(" ||||                                                                |                                                                ||||");
    }
